...
/// Provider de impressão via DeepLink (abre app de impressão externo)
/// 
...
class DeepLinkPrintAdapter implements PrintProvider {
  @override
  String get providerName => 'DeepLink Print';
  ...
  Future<PrintResult> printComanda(PrintData data) async {
    try {
      debugPrint('🖨️ Preparando impressão via DeepLink...');
      
      // Constrói o conteúdo JSON para impressão
      final printableContent = _buildPrintableContent(data);
      
      // Constrói o DeepLink
      final deepLink = _buildDeepLink(printableContent);
      
      debugPrint('🔗 Abrindo DeepLink de impressão: $deepLink');
      ...
      if (await canLaunchUrl(uri)) {
        ...
        return PrintResult(
          success: true,
          printJobId: 'DEEPLINK-PRINT-${DateTime.now().millisecondsSinceEpoch}',
        );
      } else {
        return PrintResult(
          success: false,
          errorMessage: 'Não foi possível abrir o app de impressão',
        );
      }
    } catch (e) {
      debugPrint('❌ Erro ao imprimir via DeepLink: $e');
      return PrintResult(
        success: false,
  ...
  /// Constrói o conteúdo JSON para impressão no formato esperado pelo app de impressão
  List<Map<String, dynamic>> _buildPrintableContent(PrintData data) {
  ...
  /// Constrói o DeepLink para impressão
  String _buildDeepLink(List<Map<String, dynamic>> printableContent) {
    // Converte o conteúdo para JSON string
    final jsonContent = jsonEncode(printableContent);
    
    // Padrão usado na Stone P2: printer-app://print
